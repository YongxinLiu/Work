	...
	http://210.75.224.110/report/16Sv2/human_esophagusCancer__v1
...
# 1. 处理序列 Processing sequences
	...
	# 设置工作目录
	wd=human/liyong/esophagusCancer
    ...
    # 手动编写metadata.txt
    # 隐去人名，改为PID改名
    csvtk -t headers result/metadata.txt
    tail -n+2 result/metadata.txt|cut -f 4|uniq|wc -l 
    tail -n+2 result/metadata.txt|cut -f 4|sort|uniq|wc -l 
    # 生成编号
    tail -n+2 result/metadata.txt|cut -f 4|uniq|awk '{print $0"\tp"NR}'>temp/pid.txt
    # 添加编号
	awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$2} NR>FNR{print $4,a[$4]}' temp/pid.txt result/metadata.txt > temp/name_pid.txt

	# 按样本改名
	awk 'BEGIN{OFS=FS="\t"}{system("ln seq/"$6".gz seq/"$1"_1.fq.gz");}' <(tail -n+2 result/metadata.txt)
	awk 'BEGIN{OFS=FS="\t"}{system("ln seq/"$7".gz seq/"$1"_2.fq.gz");}' <(tail -n+2 result/metadata.txt)
...
## 1.3. 样品双端合并、重命名、合并为单一文件
    ...
    for i in `tail -n+2 result/metadata.txt | cut -f 1`;do
      ...
      --fastqout temp/merge/${i}.merged.fq --relabel ${i}.
    done &
