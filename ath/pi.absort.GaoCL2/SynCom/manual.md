...
# 1. 处理序列 Processing sequences
	...
	# 设置工作目录
	wd=ath/pi.absort.GaoCL2/SynCom
...
## 1.10. 生成OTU表
	...
	# De novo: 15018601 / 22337214 mapped to OTUs (67.2%)
    # Ref: 13229114 / 22337214 mapped to OTUs (59.2%)                  
	make otutab_create
...
# 鉴定ASV与代表菌的对应关系：参考/mnt/bai/yongxin/medicago/AMF4/SynCom1FlowPotDenovo/manual.sh

    cp ../SynComRef/result/otu.fa result/ref.fa
    ...
    grep -c '>' result/ref.fa # 48
    wc -l temp/ref.blastn # 48
    # 计算OTU表相对丰度和均值
    cp /mnt/bai/yongxin/medicago/AMF4/SynCom1FlowPotDenovo/result/SynCom1SandDenovo.Rmd result/
    # 添加菌对应的最优OTU和丰度
    awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$0} NR>FNR{print a[$2]}' result/otutab_mean.txt temp/ref.blastn | sort|uniq > temp/ref.blastn.ra
    awk '{a=a+$2} END {print a}' temp/ref.blastn.ra # 79.54%
    # 存在多对1的问题？查看1对多的菌
    ...
    # 最高OTU_4 3.58
    grep -P "\tOTU_4\t" temp/ref.blastnall | sort -k3,3nr | head -n1 # less -S 
