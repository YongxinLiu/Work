...
## 筛选菌图 2019/12/12
    cd ~/ath/pi.absort.GaoCL/graphlan
    input=3_Root_isolates_WGS_testedinCGL.txt
    # 格式调整
    sed -i '/^$/d' ${input}
    # 物种注释和格式调
    usearch10 -sintax ${input} \
        -db /mnt/bai/public/ref/rdp/rdp_16s_v16_sp.udb -sintax_cutoff 0 -strand both \
        -tabbedout ${input}.tax -threads 10
    cut -f 1,4 ${input}.tax \
    ...
    sed 's/;/\t/g;s/.__//g;' otus.tax|cut -f 1-8 | \
      ...
      > taxonomy.txt
    # 统计是否第一名
    cut -f 1 taxonomy.txt|wc -l
    cut -f 1 taxonomy.txt|sort|uniq|wc -l
    
    # 按参考流程绘制
    cp ~/github/Note/R/format2graphlan/format2graphlan.Rmd ./
    cp -r ~/github/Note/R/format2graphlan/cfg ./
