...
# 1. 处理序列 Processing sequences
    ...
    cp ../strigolactone.LiJY/doc/library.txt doc/
    mkdir -p raw
    awk 'BEGIN{OFS=FS="\t"}{system("ln -s /mnt/bai/yongxin/rice/strigolactone.LiJY/clean_data/"$1"_1.fq.gz raw/"$1"_1.fq.gz");}' <(tail -n+2 doc/library.txt )
    awk 'BEGIN{OFS=FS="\t"}{system("ln -s /mnt/bai/yongxin/rice/strigolactone.LiJY/clean_data/"$1"_2.fq.gz raw/"$1"_2.fq.gz");}' <(tail -n+2 doc/library.txt )
    # 转换64为33
    # fastp -i raw/L1_1.fq.gz -I raw/L1_2.fq.gz -o seq/L1_1.fq.gz -O seq/L1_2.fq.gz -6 -A -G -Q -L -w 8
    for i in `tail -n+2 doc/library.txt|cut -f 1`; do
      fastp -i raw/${i}_1.fq.gz -I raw/${i}_2.fq.gz -o seq/${i}_1.fq.gz -O seq/${i}_2.fq.gz -6 -A -G -Q -L -w 8 &
    done
    # 检查数据链接，全红为错误，绿色为正常
    ll seq/*
	# 如果压缩文件，要强制解压链接
	...
	# 从其它处复制实验设计
	cp ~/rice//strigolactone.LiJY/doc/L*.txt doc/
	# 删除多余空格，windows换行符等(苹果用户勿用)
	sed -i 's/ //g;s/\r//' doc/*.txt 
	head -n3 doc/L9.txt
	# 依据各文库L*.txt文件生成实验设计
	cat <(head -n1 doc/L9.txt | sed 's/#//g') <(cat doc/L* |grep -v '#'|grep -v -P '^SampleID\t') > doc/design.txt
	# 检查是否相等
	wc -l doc/design.txt
