...
# 1. 处理序列 Processing sequences
	...
	cp ~/rice/miniCore/doc/library.txt doc/library.txt
	# 按library中第二列index准备测序文库，如果压缩要添加.gz，并用gunzip解压
    mkdir -p raw
    awk 'BEGIN{OFS=FS="\t"}{system("ln -s /mnt/bai/yongxin/rice/miniCore/clean_data/"$1"_1.fq.gz raw/"$1"_1.fq.gz");}' <(tail -n+2 doc/library.txt )
	awk 'BEGIN{OFS=FS="\t"}{system("ln -s /mnt/bai/yongxin/rice/miniCore/clean_data/"$1"_2.fq.gz raw/"$1"_2.fq.gz");}' <(tail -n+2 doc/library.txt )
    # 转换64为33
    fastp -i raw/L1_1.fq.gz -I raw/L1_2.fq.gz -o seq/L1_1.fq.gz -O seq/L1_2.fq.gz -6 -A -G -Q -L -w 8
    for i in `seq 2 40`; do
      fastp -i raw/L${i}_1.fq.gz -I raw/L${i}_2.fq.gz -o seq/L${i}_1.fq.gz -O seq/L${i}_2.fq.gz -6 -A -G -Q -L -w 8 &
    done
    # 检查数据链接，全红为错误，绿色为正常
    ll seq/*
	# 如果压缩文件，要强制解压链接
	...
	# 从其它处复制实验设计
	cp ~/rice/miniCore/doc/L*.txt doc/
