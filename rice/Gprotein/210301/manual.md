...
# 1. 处理序列 Processing sequences
	...
	head -n3 doc/L01.txt
	# 依据各文库L*.txt文件生成实验设计
	cat <(head -n1 doc/L01.txt | sed 's/#//g') <(cat doc/L* |grep -v '#'|grep -v -P '^SampleID\t') > doc/design.txt
	# 检查是否相等
	wc -l doc/design.txt
...
## 1.6. 序列去冗余
	...
	# 推荐 minuniquesize 筛选标准最小为百万分之一(1~5/M)，如如最后一行输出数据量53M，阈值为53
	make fa_unique
