...
# 2. 统计绘图
	...
	g1_list='"HTEJ","HIND","HSoil1","LTEJ","LIND","LSoil1"'
	...
	legend_number=10
	# 差异统计按丰度过滤 abundance filter，如丰度按每组最位数大于万分之一过滤，减少计算量，降低OTU的FDR值，可选千一，或千万
	...
	doc=doc/${sub}
	# 报告输出目录
	version=xiangeng${sub}${compare_method}4
...
## 2.3 beta_pcoa 主坐标轴分析距离矩阵 PCoA of distance matrix "bray_curtis","binary_jaccard","weighted_unifrac","unweighted_unifrac"
	...
	bp_method='"bray_curtis","weighted_unifrac"'
	bp_design=${design}
	bp_group_name=${g1}
