...
# 1. 处理序列 Processing sequences
	...
	# -i 's/\t/\tL171121_/' doc/library.txt # time check SeqLibraryList.xlsx
	...
	# 检查是否相等，也与实验设计一致
	wc -l doc/design.txt # 908-1=907
	cut -f 1 doc/design.txt|sort|uniq|wc -l
    # 检查时间点是否正确，与设计不一致的修正
    cut -f 10 doc/design.txt | sort | uniq -c | sort -k2,2n|tail -n+2|wc -l # 查看dayMod是否为22个与design一致
    cut -f 10 doc/design.txt | sort | uniq -c | sort -k2,2n # 不一致的修正
    cut -f 16 doc/design.txt | sort | uniq -c | sort -k2,2n # |tail -n+2|wc -l
...
## 1.2. 按实验设计拆分文库为样品
	...
	# 统计结果见result/split有txt/pdf/png，推荐看png方便快速查看每张位图，结果数量正常是样本量2倍
	# 查看样本量排序，2个样本极少，剩下的都3万以上
