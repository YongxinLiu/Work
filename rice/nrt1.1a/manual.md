...
# 1. 处理序列 Processing sequences
	...
	# 保存Excel模板中第一页basic中第三部分测序文库列表library为doc/library.txt
	# 按library中第二列index准备测序文库，注意文件是否压缩、名称是Index正向$5还是反向互补$2；此次是压缩且反向互补；压缩要去掉-s用硬链才能解压
	awk 'BEGIN{OFS=FS="\t"}{system("ln /mnt/bai/yongxin/seq/180719.nrt1.1a.lane12/Clean/AC/"$5"_1.fq.gz seq/"$1"_1.fq.gz");}' <(tail -n+2 doc/library.txt )
	awk 'BEGIN{OFS=FS="\t"}{system("ln /mnt/bai/yongxin/seq/180719.nrt1.1a.lane12/Clean/AC/"$5"_2.fq.gz seq/"$1"_2.fq.gz");}' <(tail -n+2 doc/library.txt )
	# 压缩文件需解压，硬链需加-f才可以强制解压
	gunzip -f seq/*
	...
	# 检查样本是否重名：样本数与唯一样本数是否相等
	wc -l doc/design.txt
	cut -f 1 doc/design.txt|sort|uniq|wc -l
	# 检查样品名在水稻之前研究中名称是否唯一，两者相差1是正确的
	cat doc/design.txt ~/rice/xianGeng/doc/design.txt|wc -l
	cat doc/design.txt ~/rice/xianGeng/doc/design.txt|cut -f 1|sort|uniq|wc -l
