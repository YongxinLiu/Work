...
# 1. 处理序列 Processing sequences
	...
	head -n3 doc/L5.txt
	# 依据各文库L*.txt文件生成实验设计
	cat <(head -n1 doc/L5.txt | sed 's/#//g') <(cat doc/L* |grep -v '#'|grep -v -P '^SampleID\t') > doc/design.txt
	# 检查是否相等
	wc -l doc/design.txt
...
## 1.2. 按实验设计拆分文库为样品
	...
	# 调用perl时版本错误，parallel无法运行，手动调perl5lib即可，或conda deactivate
...
## 1.17. 有参考构建OTU表-非16S不可用
    ...
    # 复制文件模板，手工制作
    cp ../SynCom2Sand/doc/compare.txt doc/
    cp ../SynCom2Sand/doc/venn.txt doc/

    # 绘制Bacillus属的丰度
    alpha_boxplot.sh \
      -i `pwd`/result/tax/sum_g.txt -m '"Bacillus","Pseudomonas"' \
      -d `pwd`/doc/design.txt -A GroupID -B `cat doc/""/compare.txt|tr '\t' '\n'|sort|uniq|awk '{print "\""$1"\""}'|tr "\n" ","|sed 's/,$//'` \
      -o `pwd`/result/otu_boxplot/ -h 6 -w 8 -t TRUE -n F
    ...
    # 最高OTU_13 1，OTU_14 0.86
    grep -P "\tOTU_13\t" temp/ref.blastnall | sort -k3,3nr | head -n1 # less -S 
    grep -P "\tOTU_14\t" temp/ref.blastnall | sort -k3,3nr | head -n1 #  less -S 
