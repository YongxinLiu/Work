...
# 1. 处理序列 Processing sequences
	...
	# 设置工作目录
	wd=medicago/AMF4/SynCom1SandDenovo
    ...
    grep -v 'Syncom2' ../SynCom1Sand/doc/L1.txt > doc/L1.txt 
    grep -v 'Syncom2' ../SynCom1Sand/doc/L2.txt > doc/L2.txt 
...
## 1.10. 生成OTU表
	...
	make otutab_create
    # 5270781 / 6415846 mapped to OTUs (82.2%)
    # 比对至参考SynCom1 5174350 / 6415846 mapped to OTUs (80.6%)
    usearch10 -otutab temp/filtered.fa -otus ../SynCom1Sand/result/otu.fa -id 0.97 \
        -otutabout temp/syncom1_otutab.txt -threads 48
    # 比对至参考SynCom2 4538241 / 6415846 mapped to OTUs (70.7%)
...
# 鉴定ASV与代表菌的对应关系
    ...
    awk '{a=a+$2} END {print a}' temp/ref.blastn.ra # 97.87%
    # 34菌对应的OTU丰度超过100%，存在多对1的问题？查看1对多的菌
    cut -f 2 temp/ref.blastn|sort|uniq -d # OTU_11，OTU_13，OTU_32，OTU_95。均有一个高一个低相似度。
    # 查看非目标菌的丰度
    ...
    # 最高OTU_16 0.7，OTU_24 0.3，OTU_29 0.14，OTU_30 0.11%
    grep -P "\tOTU_16\t" temp/ref.blastnall | sort -k3,3nr | head -n1 # less -S 
    grep -P "\tOTU_24\t" temp/ref.blastnall | sort -k3,3nr | head -n1 #  less -S 
    grep -P "\tOTU_29\t" temp/ref.blastnall | sort -k3,3nr | head -n1 #  less -S 
    grep -P "\tOTU_30\t" temp/ref.blastnall | sort -k3,3nr | head -n1 #  less -S 
    # 查找其与添加菌的相似关系，OTU_16最高与cOTU_60 90.34%；OTU_24，OTU_30也只有90.34和90.39%；OTU_29相似度97.85%
