...
# 1. 处理序列 Processing sequences
	...
	# 设置工作目录
	wd=medicago/AMF4/SynCom2SandDenovo
    ...
    grep -v 'Syncom1' ../SynCom2Sand/doc/L3.txt > doc/L3.txt 
    grep -v 'Syncom1' ../SynCom2Sand/doc/L2.txt > doc/L2.txt 
...
#	gunzip -f seq/*.gz
	...
	head -n3 doc/L2.txt
	# 依据各文库L*.txt文件生成实验设计
	cat <(head -n1 doc/L2.txt | sed 's/#//g') <(cat doc/L* |grep -v '#'|grep -v -P '^SampleID\t') > doc/design.txt
	# 检查是否相等
	wc -l doc/design.txt
...
## 1.3. 样品双端合并、重命名、合并为单一文件
    ...
    for i in `tail -n+2 doc/design.txt|cut -f 1`; do
        # i=SdP0Syncom1R108R1
        ln -sf /mnt/bai/yongxin/medicago/AMF4/SynCom2Sand/temp/merge/${i}.fq temp/merge/
    done
...
## 1.8. 有参去嵌合体
	...
	# 基于SILVA数据库去除
	make chimera_ref
    # 检查是否会去掉真的序列，Chimeras 279/754 (37.0%)
...
## 1.10. 生成OTU表
	...
	make otutab_create
    # 5538414 / 6573006 mapped to OTUs (84.3%)
    # 比对至参考SynCom1 5104921 / 6573006 mapped to OTUs (77.7%)
    usearch10 -otutab temp/filtered.fa -otus ../SynCom1Sand/result/otu.fa -id 0.97 \
        -otutabout temp/syncom1_otutab.txt -threads 48
    # 比对至参考SynCom2 5390475 / 6573006 mapped to OTUs (82.0%)
    usearch10 -otutab temp/filtered.fa -otus ../SynCom2Sand/result/otu.fa -id 0.97 \
        -otutabout temp/syncom2_otutab.txt -threads 48
    # 结果还是往SynCom1上结果更好，高10%应该为正常
...
# 鉴定ASV与代表菌的对应关系
    ...
    cp ../SynCom2Sand/result/otu.fa result/ref.fa
    ...
    grep -c '>' result/ref.fa # 24
    wc -l temp/ref.blastn # 24
    # 计算OTU表相对丰度和均值
    result/SynCom2SandDenovo.Rmd
    # 添加菌对应的最优OTU和丰度
    awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$0} NR>FNR{print a[$2]}' result/otutab_mean.txt temp/ref.blastn | sort|uniq > temp/ref.blastn.ra
    awk '{a=a+$2} END {print a}' temp/ref.blastn.ra # 96.1%
    # 查看1对多的菌
    cut -f 2 temp/ref.blastn|sort|uniq -d # OTU_15
    # 查看非目标菌的丰度
    ...
    # 最高OTU_12 1.48，OTU_315 0.36，OTU_20 0.25，千分之一以上7个
    grep -P "\tOTU_12\t" temp/ref.blastnall | sort -k3,3nr | head -n1 # less -S 
    grep -P "\tOTU_315\t" temp/ref.blastnall | sort -k3,3nr | head -n1 #  less -S 
    grep -P "\tOTU_20\t" temp/ref.blastnall | sort -k3,3nr | head -n1 #  less -S 
    # 查找其与添加菌的相似关系，OTU_12最高与cOTU_22 96.58%；OTU_315最高与cOTU_24 98.67%；OTU_20最高与cOTU_45 99.74%；已经菌相似，可能为分歧。
